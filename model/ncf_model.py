...
class MLP(nn.Module):
    ...
    def __init__(self, user_size, item_size, hidden_size, layers: list = [64,32,16,8]):
        super().__init__()
...
class NeuralMF(TorchInterfaceRecomm):
    ...
    def __init__(self, user_size: int, item_size: int, hidden_size: int, layers: list = [64,32,16,8], component: list = None):
        super().__init__()
    ...
    def forward(self, user, item):
        ...
        for k in self.component:
            vectors.append(self.model_component[k](user, item))

        x = torch.concat(vectors, dim=-1)

        return self.output_layer(x).reshape(-1)

    def _compute_loss(self, data: Iterable, loss_func, optimizer=None, scheduler=None, train=True) -> Tensor:
    ...
    def _validation(self, test_dataloader, loss_func):
        self.eval()
        ...
        with torch.no_grad():
            for step, data in enumerate(test_dataloader):
                ...
                _, indices = torch.topk(y_hat, k=10)
